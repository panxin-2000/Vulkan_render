...
int graph_square_y;
int graph_square_x;
// 新增的 graph_square_width 并不影响 get_vertex

vertex &get_vertex(const std::vector<vertex *> &graph, int x, int y) {
    return *graph.at(x + y * graph_square_x);
}

vertex &get_vertex(const std::vector<vertex *> &graph, const vertex_position vertex) {
    return *graph.at(vertex.x_position + vertex.y_position * graph_square_x);
}

std::list<vertex_position> &get_vertex_edge_list(const std::vector<vertex *> &graph, int x, int y) {
    return graph.at(x + y * graph_square_x)->edge_list;
}

void add_edge(std::vector<vertex *> &graph, int x, int y, enum direction d) {
    auto &edge_list = get_vertex_edge_list(graph, x, y);
    if (d == up_x_decrease && x != 0) {
        add_edge(edge_list, x, y, up_x_decrease);
        auto &edge_list_2 = get_vertex_edge_list(graph, x - 1, y);
        add_edge(edge_list_2, x - 1, y, down_x_add);
    } else if (d == down_x_add && x != graph_square_y - 1) {
        add_edge(edge_list, x, y, down_x_add);
        auto &edge_list_2 = get_vertex_edge_list(graph, x + 1, y);
        add_edge(edge_list_2, x + 1, y, up_x_decrease);
    } else if (d == left_y_decrease && y != 0) {
        add_edge(edge_list, x, y, left_y_decrease);
        auto &edge_list_2 = get_vertex_edge_list(graph, x, y - 1);
        add_edge(edge_list_2, x, y - 1, right_y_add);
    } else if (d == right_y_add && y != graph_square_x - 1) {
        add_edge(edge_list, x, y, right_y_add);
        auto &edge_list_2 = get_vertex_edge_list(graph, x, y + 1);
        add_edge(edge_list_2, x, y + 1, left_y_decrease);
    }
}
    ...
    if (d == up_x_decrease) {
        for (const auto tem: edge_list) {
            if (tem.x_position == x - 1 && tem.y_position == y) {
                ...
                delete_edge(graph, x - 1, y, down_x_add);
                break;
            }
        }
    } else if (d == down_x_add) {
        for (const auto tem: edge_list) {
            if (tem.x_position == x + 1 && tem.y_position == y) {
                edge_list.remove(tem);
                delete_edge(graph, x + 1, y, up_x_decrease);
                break;
            }
        }
    } else if (d == left_y_decrease) {
        for (const auto tem: edge_list) {
            if (tem.x_position == x && tem.y_position == y - 1) {
                edge_list.remove(tem);
                delete_edge(graph, x, y - 1, right_y_add);
                break;
            }
        }
    } else if (d == right_y_add) {
        for (const auto tem: edge_list) {
            if (tem.x_position == x && tem.y_position == y + 1) {
                edge_list.remove(tem);
                delete_edge(graph, x, y + 1, left_y_decrease);
                break;
            }
        }
    }
...
std::vector<vertex *> *init_graph(int square_y, int square_x) {
    graph_square_y = square_y;
    graph_square_x = square_x;
    ...
    for (int y = 0; y < square_y; ++y) {
        for (int x = 0; x < square_x; ++x) {
            ...
            if (x == 0 && y == 0) {
                // 左上角
                add_edge(list->edge_list, x, y, down_x_add);
                add_edge(list->edge_list, x, y, right_y_add);
            } else if (x == square_x - 1 && y == square_y - 1) {
                // 右下角
                add_edge(list->edge_list, x, y, up_x_decrease);
                add_edge(list->edge_list, x, y, left_y_decrease);
            } else if (x == square_x - 1 && y == 0) {
                // 左下角
                add_edge(list->edge_list, x, y, up_x_decrease);
                add_edge(list->edge_list, x, y, right_y_add);
            } else if (x == 0 && y == square_y - 1) {
                // 右上角
                add_edge(list->edge_list, x, y, left_y_decrease);
                add_edge(list->edge_list, x, y, down_x_add);
            } else if (x == 0) {
...
// 为什么不能加上 vertex_in_path.distance // 错在哪里了？
void relax_first(vertex &update_vertex, vertex &vertex_in_path) {
    auto tem_d = get_weight(&update_vertex, &vertex_in_path)
                 + abs(end_position.x_position - update_vertex.self_position.x_position)
                 + abs(end_position.y_position - update_vertex.self_position.y_position);
    if (update_vertex.distance > tem_d) {
        update_vertex.distance = tem_d;
        update_vertex.parent = &vertex_in_path;
    }
...
int distance_between(vertex_position &u, vertex_position &v) {
    return abs(u.x_position - v.x_position) + abs(u.y_position - v.y_position);
}

// A的判断是两个 从起始点到 当前点的距离，然后当前点 到 结束点 的预判距离。
// 比较的时候比较的是上面的，但是更新的时候更新的是什么呢？
// 应该是这样的，但是现在没有阻挡，没有办法测试看看，还需要改改交互来测试
void relax_A(vertex &update_vertex, vertex &vertex_in_path) {
    auto new_distance = vertex_in_path.distance + get_weight(&update_vertex, &vertex_in_path);
    if (update_vertex.distance + distance_between(update_vertex.self_position, end_position) >
        new_distance + distance_between(vertex_in_path.self_position, end_position)) {
        update_vertex.distance = new_distance;
        update_vertex.parent = &vertex_in_path;
    }
}
...
void dijkstra(const std::vector<vertex *> &graph, vertex_position start, vertex_position end) {
    ...
    int i = 0;
    for (const auto &it: graph) {
        it->distance = 500; // 初始距离随便设，距离的平方也是可以的
        it->parent = nullptr;
    }
    ...
    while (vertices_priority_queue.size() > 0) {
        ...
        for (auto v: edge) {
            relax(get_vertex(graph, v.x_position, v.y_position), *u);
        }
    }
}

vertex *dijkstra_step(const std::vector<vertex *> &graph, std::vector<vertex *> &vertices_priority_queue) {
    if (vertices_priority_queue.size() > 0) {
        std::sort(vertices_priority_queue.begin(), vertices_priority_queue.end(),
                  [](vertex *left, vertex *right) { return left->distance > right->distance; });
        auto u = vertices_priority_queue.back(); // 这里需要去更新显示的数据
        vertices_priority_queue.pop_back();
        auto edge = u->edge_list;
        for (auto v: edge) {
            relax_first(get_vertex(graph, v.x_position, v.y_position), *u);
        }
        return u;
    } else {
        return nullptr;
