...
//
highp vec3 computeViewSpacePositionFromDepth(highp vec2 uv,
...
highp vec3 computeViewSpaceNormalMediumQ(
        const highp sampler2D depthTexture,
        const highp vec2 uv,
...
// uv             : normalized coordinates                      当前像素的屏幕 UV 坐标
// depth          : linear depth at uv                         当前像素的原始深度值（Scalar float）
// position       : view space position at uv                 当前像素在观察空间（View Space）中的三维坐标（vec3）
