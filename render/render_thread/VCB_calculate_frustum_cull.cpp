    ...
    for (uint32_t pass = 0; pass < 6; pass++) {
        ...
        bool isEven           = (pass % 2 == 0); {
            ...
            vkCmdPushConstants(command_buffer_, command_shader->pipeline_layout,
                               ...
                               sizeof(RadixHistogramPushConstants),
                               &radixPC);
            vkCmdDispatch(command_buffer_, numWorkgroups, 1, 1);
            // 这里比较简单 ,
            add_buffer_write_to_read_barriers({histograms});
        } {
            VKR_shader_paths temp{
                "", "", "", "3DGS/radixsort"
            };
            ...
            vkCmdDispatch(command_buffer_, numWorkgroups, 1, 1);
            if (isEven == false) {
                add_buffer_write_to_read_barriers({keysRadix, valuesRadix});
            } else {
                add_buffer_write_to_read_barriers({keys, values});
            }
        }
    }
}
...
void VCB::add_buffer_read_to_write_barriers(const std::vector<VKR_buffer_ptr> &buffer_ptrs) const {
