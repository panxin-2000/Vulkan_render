...
namespace utils {
    ...
    class StructureOfArraysBase {
        // number of elements // 变长参数包（Parameter Pack）中包含的元素个数
    ...
    public:
        ...
        //  std::tuple_element_t 查找元组 Structure 中索引为 N（从 0 开始计数）的元素类型
        ...
        void push_back_unsafe(Elements &&... args, ElementIndices<Indices...>) {
            size_t last = mSize++;
            // Fold expression on the comma operator 在编译期自动按顺序生成多个数组的插入代码
            ([&] {
    ...
    private:
        // 下面两个是很 好玩的事情， 巧妙的 I 为 sizeof...(Tp) 的时候函数非空
        // 其他的时候都是 递归的 向上更新 I 值并调用
        ...
        void move_each(void *buffer, size_t capacity) noexcept {
            ...
            // update the pointers 看起来应该是这里更新了 mArrays
