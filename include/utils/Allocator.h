...
namespace utils {
     ...
     *
     * 在 LinearAllocator 分配失败时 回退 到 HeapAllocator 分配器
     * reset or destroyed 时才会  free 申请到 heap 内存区域
...
#define UTILS_MAX(a,b) ((a) > (b) ? (a) : (b))
    ...
    // 这里的意思应该是区域策略，从哪里分配内存的。 给一个固定大小的内存，
    // 主要的函数有四个，data，begin，end，size
    // 之后应该也只能用这个四个函数，还有另一个的重要的就是构造和析构函数了
    // 从最后一个看的话，data 和 size 这个两个函数是会被使用的
    // 但是实际上 ，begin 和 end 是被使用的，作为 AREA 提供给 LinearAllocator ,
    // heapAllocator 直接忽视这个
    ...
    // 这里的意思是追踪的策略
    // 主要是四个函数 onAlloc onFree onReset onRewind
    // 最后一个 onRewind 是 倒带，重播的意思
    ...
    // 四种策略，
    // 第一个是内存 是从 LinearAllocator 或者  HeapAllocator 中申请的
    //                PoolAllocator         PoolAllocatorWithFallback
    // PoolAllocator 应该被理解 池分配策略，
    // PoolAllocatorWithFallback 应该被理解为池分配失败时就回退到堆分配器
    // 选择分配的区域 是从一个固定大小的缓冲区 逐渐向上分配还是 从系统内存中直接申请
    // 第二个是申请内存时 是否加锁
    // 第三个是关于是否记录的并追踪debug的
    // 第四个 有 HeapArea  staticArea noArea  是向 第一个 提供 内存区域的
    ...
    // 这里对上面的 Arena 进行了再次的封装
    // 知道了是再次封装，但是多了哪些内容并没有看
    ...
    // 也是再次的封装，甚至还多了一个 TYPE ，用于固定大小的元素的申请
