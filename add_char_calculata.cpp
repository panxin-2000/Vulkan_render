...
struct tree_node_calculate {
    ...
    int tree_add_after_node(struct tree_node_calculate *new_node) {
    }
...
struct tree_node_calculate *construction_calulate_tree(int argc, char **argv) {
    if (argc == 0) {
        ...
        struct tree_node_calculate *root;
        for (int i = 1; i < argc; i++) {
            int string_length = strlen(argv[i]);
            if (string_length == 1 && isascii(argv[i][0])) {
                // 这里判断是字符的情况
                for (int i = 0; i < sizeof(operate_symbol_and_functions) / sizeof(operate_symbol_and_function); i++) {
                    if (operate_symbol_and_functions[i].symbol == argv[i][0]) {
                        struct tree_node_calculate *new_node = init_a_calculate_note(
                            0, operate_symbol_and_functions[i].symbol);
                        root->tree_add_after_node(new_node);
