...
void *rt_memheap_alloc(struct rt_memheap *heap, rt_size_t size)
{
    ...
    if (size < heap->available_size)
    {
        ...
        /* lock memheap */
        if (heap->locked == RT_FALSE)   // 这里是不是有问题，如果被改了，会怎么样呢？这里应该就是单线程和多线程的区别了
        {
        ...
        while (header_ptr != heap->free_list && free_size < size) //为什么会这个样子呢？因为list是可以循环的
        {
...
void *_memheap_alloc(struct rt_memheap *heap, rt_size_t size)
{
    ...
    if (ptr == RT_NULL)   // 这里只是一个调用了，没有什么大不了的
    {
