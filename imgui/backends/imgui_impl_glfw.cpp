...
void ImGui_ImplGlfw_CursorPosCallback(GLFWwindow* window, double x, double y)
{
    ...
    if (bd->PrevUserCallbackCursorPos != nullptr && ImGui_ImplGlfw_ShouldChainCallback(window))
        bd->PrevUserCallbackCursorPos(window, x, y);    // 我不懂这里是什么意思，应该是和glfwSetCursorPosCallback
                                                                    // 的返回值是有关的
                                                                    // The previously set callback, or `NULL` if no callback was set or the library had not been [initialized]
                                                                    // 确实是和返回值有关的 然后就是窗口的问题？ 不清楚这里的window发生了什么？多个窗口？
...
void ImGui_ImplGlfw_RestoreCallbacks(GLFWwindow* window) // 没看懂这个函数是做什么的，或者说
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
...
static bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    ...
    io.SetClipboardTextFn = ImGui_ImplGlfw_SetClipboardText;  // glfwSetClipboardString((GLFWwindow*)user_data, text);  实际调用的是这个
    io.GetClipboardTextFn = ImGui_ImplGlfw_GetClipboardText;  // glfwGetClipboardString((GLFWwindow*)user_data);        实际调用的是这个
