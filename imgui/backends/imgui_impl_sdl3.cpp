...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event *event) {
    ImGui_ImplSDL3_Data *bd = ImGui_ImplSDL3_GetBackendData();   // 这里得到的结果是 NULL ， 需要看看我之前是如何处理的
...
static void ImGui_ImplSDL3_SetupPlatformHandles(ImGuiViewport *viewport, SDL_Window *window) {
    ...
    viewport->PlatformHandleRaw = (HWND) SDL_GetPointerProperty(SDL_GetWindowProperties(window),
...
void ImGui_ImplSDL3_SetMouseCaptureMode(ImGui_ImplSDL3_MouseCaptureMode mode) {
    ImGui_ImplSDL3_Data *bd = ImGui_ImplSDL3_GetBackendData();
    if (mode == ImGui_ImplSDL3_MouseCaptureMode_Disabled && bd->MouseCaptureMode !=
...
void ImGui_ImplSDL3_NewFrame() {
    ...
    io.DeltaTime = bd->Time > 0
                       ? (float) ((double) (current_time - bd->Time) / (double) frequency)
                       : (float) (1.0f / 60.0f);
    bd->Time = current_time;

    if (bd->MousePendingLeaveFrame && bd->MousePendingLeaveFrame >= ImGui::GetFrameCount() && bd->MouseButtonsDown ==
        0) {
        bd->MouseWindowID          = 0;
        bd->MousePendingLeaveFrame = 0;
