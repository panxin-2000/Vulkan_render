...
struct ImVector
{
    int                 Size;
    int                 Capacity;
    T*                  Data;       // 这里是一个指针，位置是一块内存区域，问题是这个是在哪里申请的？

    // Provide standard typedefs but we don't use them ourselves.
    // 提供标准类定义，但是我么不使用它
    // 其实看这里是在看如何设计一个vector的
...
struct ImDrawList
{
    ...
    ImVector<ImDrawVert>    VtxBuffer;          // Vertex buffer.   // 这里就是顶点和索引，问题是怎么画的？
    ...
    ImVector<ImVec2>        _Path;              // [Internal] current path building //每次绘制完线之后都会被清零  也忘了是在哪里了
