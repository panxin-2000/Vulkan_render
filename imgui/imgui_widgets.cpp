...
//    text_pos 就是左上的位置了
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        ImRect bb(text_pos, text_pos + text_size); // 包围盒是左上和右下的两个位置
        ItemSize(text_size, 0.0f);
        if (!ItemAdd(bb, 0))  // 为什么这里ID是零，是因为不用做判断吗？
            return;
...
// 这里添加一个是什么意思呢，有一条更新记录，va_list variant 的意思
void ImGui::TextV(const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();  // windows 都是现场获得的。
    if (window->SkipItems)
        return;
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    bool pressed = false;
    bool hovered = ItemHoverable(bb, id, item_flags);  // 这里判断鼠标是否在相应的位置上 ，但是里面还又套了两层还是三层
    ...
    // test_owner_id 默认情况下它就是id，不清楚 ImGuiButtonFlags_NoTestKeyOwner  添加这个会有什么影响 ImGuiKeyOwner_Any = 0
    if (hovered)
    {
        ...
        for (int button = 0; button < 3; button++) // 不知道这里为什么是 3  原因是因为 MouseButton 有 3 个， 左 中 右
    ...
    if (g.NavHighlightActivatedId == id) // 这个注释不对 ->  这里是判断键盘是否到当前区域的一个办法
        hovered = true;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ItemSize(total_bb, style.FramePadding.y);
    const bool is_visible = ItemAdd(total_bb, id);  // 这里的ID不是零，确实是在没有需要判断点击区域的时候是没有ID的。
                                                        // ID 是很重要的，因为每个BB（bounding-box）在添加的时候都是有ID的，非常清楚的知道了包围盒的重要性。
                                                        // 那么有一个新的问题，3D的时候的包围盒？
    if (!is_visible)
    {
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, id, &hovered, &held);// ButtonBehavior
    if (pressed) // 这里的 pressed 其实是表示了按下后松开的结果。 hovered  held 这两个参数对应的那些内容我还是不是很清楚。
                 // hovered 是盘旋的，也就是如果鼠标到达相应的bounding-box后就会触发
                 // held 是键盘按下后才会触发  // 这两个都true的情况下也不一定 设置
    {
        *v = !(*v);
        MarkItemEdited(id);
    }
    ...
    if (is_visible)
    {
        RenderNavHighlight(total_bb, id); // 如果鼠标到这个包围盒上，渲染一个长方形和边界
        RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
//       上面这一行附带了高亮的功能，GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg)
//       不需要再获取一次坐标来解决是否需要高亮
        ...
        else if (*v)
        {
            ...
            RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f); // 这个是绘制对号标记的
        }
    }
    ...
    if (is_visible && label_size.x > 0.0f)
        RenderText(label_pos, label);  //确实先渲染前面的方块，再渲染之后的文字的
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label); // 这个应该是跟hash是有关的，每个应该都是不一样的，
    const float w = CalcItemWidth();             // 计算宽度
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb); // SliderBehavior
    if (value_changed)
        MarkItemEdited(id);
